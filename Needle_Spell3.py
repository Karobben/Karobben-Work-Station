...
# Quick start
# python3 Needle_Spell3.py FILE (text file with the words or sentences separate by "\n")
...
def WorS(INPUT,Result):
    global List_N
    Result_t = np.array([0]*7)
    if len(INPUT.split(" ")) == 1:
        A,B,C = Word_RC(Result)
        Result_t[0] += 1
        Result_t[1] += 1
        Result_t[2] += C
...
def Mark(Seq1,Seq2):
    ...
    for i in range(len(Seq1)):
        if Seq1[i] != Seq2[i]:
            Mark[i] = "X"
            Seq2[i] = "\x1b[3;45;6m"+Seq2[i]+"\x1b[0m"
...
def run(Seq):
    ...
    Scores = []
    for i in  AA:
        String,score = main(i, Seq)
        Result += [String]
        Scores += [score]
    return Result[Scores.index(max(Scores))]
...
while True:
    ...
    if Seq == "q":
        ...
        if Seq in AA:
            Num +=1
            print("You Are Right!!",Num,"\n\n")
            os.system("echo "+Seq+" >tmp.word")
